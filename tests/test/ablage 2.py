...
This implementation provides a proper pipeline for DHM reconstruction with several key features:

1. **Complex Field Reconstruction**:
   - Proper FFT/IFFT handling with shifting
   - Circular mask for first order isolation
   - First order centering

2. **Intensity Calculation**:
   - Correct |E|²  calculation for linear intensity
   - Proper dB conversion if needed (using 10*log10 for intensity)
   - Small epsilon to avoid log(0)

3. **Phase Calculation**:
   - Proper phase extraction using np.angle()
   - Option for phase unwrapping

The key differences from your previous implementation:

1. The intensity is now calculated as |E|² directly, which is always non-negative
2. When using dB scale, it uses 10*log10 instead of 20*log for intensity (20*log is for amplitude)
3. Added proper handling of the reconstruction steps

To use this with your hologram:
...
complex_field = reconstructor.reconstruct_hologram(
    your_hologram,
    mask_radius=your_radius,
    center_coords=your_center
)

# Get intensity (linear scale)
intensity = reconstructor.calculate_intensity(complex_field, mode='linear')
...
Would you like me to explain any specific part in more detail or add additional functionality for your specific needs? Also, if you're seeing specific artifacts in your reconstruction, we can add additional processing steps to address them.
"""
