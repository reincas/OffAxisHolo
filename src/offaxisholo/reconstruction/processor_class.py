...
from .phaseUnwrapping import (
    phase_unwrapping_fast2d,
    phase_unwrapping_numpy,
)  # , phase_unwrapping_kamui_normal
...
class HologramProcessor(HologramCore):
    ...
    def __init__(
        self,
        hologram: Hologram,
        reference: ReferenceHologram = None,
        dhm_parameter: dict = None,
        material_parameter: dict = None,
        logger=None,
        saving_path=None,
    ):
        ...
        self.params.update(
            {"MaterialDictionary": material_parameter}
        ...
        try:
            self.refractiveIndex = (
                material_parameter["refractiveIndex"]
                if material_parameter is not None
                else self.refractive_index_default
            )
    ...
    def plot_reconstruction(
        ...
        if mode.lower() == "short":
            self.plotter.plot_reconstruction_short(
                self,
                show_plot=show_plot,
            ...
            self.plotter.plot_full_reconstruction_process(
                self,
                show_plot=show_plot,
                save_single=save_single,
                cmap=cmap,
    ...
    def run(
        self,
        hologram: Hologram = None,
        background_hologram: ReferenceHologram = None,
        *,
        prop_dist=None,
        propagate=True,
        compensate=True,
        compensation_mode: Literal["Background", "ZernikePolynomial"] = "Background",
        filtering=False,
        filter_applied: list = None,  # todo think of a better name for the list
        ...
        self.logger.debug(
            f"Hologram processing with {mode} mode."
        ...
        if compensate:
            if compensation_mode == "Background":
                self.logger.info("Starting compensation with the background field...")
                # Reconstruction of Background
                background_field = background_hologram.reconstruct()

                self.compensate(
                    original=self.field_propagated, reference=background_field
                )
                # Note: recalculated field is not correct (while printing)

                self.logger.info(
                    f"Starting phase unwrapping with {phase_unwrapping_method} method."
                )
                if mode.lower() == "print":
                    self.phase_map = self.phase_unwrapping(
                        self.phase_compensated, method=phase_unwrapping_method
                    )
                ...
                raise NotImplementedError(
                    f"Compensation method {compensation_mode} not implemented."
                )
        else:
            self.phase_map = self.phase_unwrapping(
                self.phase(self.field_propagated), method=phase_unwrapping_method
            )
            self.intensity_reconstructed = self.intensity(
                self.field_propagated, mode="db"
            )
        ...
        if filtering:
            ...
            for i in range(len(filter_applied)):
                ...
                if filter_name.lower() == "gauss" or filter_name.lower() == "gaussian":
                    ...
                    filter_name.lower() == "hm2f"
                    or filter_name.lower() == "hybrid mean-median filter"
                ):
                    pass
    ...
    def propagate(
        ...
        if propagation_method == "angularSpectrum" and float(distance) != 0.0:
            ...
            raise NotImplementedError(
                f"No propagation method {propagation_method} implemented."
            )

        self.logger.info("Propagation finished ...")
        return propagated

    def compensate_phase(
    ...
    def compensate(self, original: np.ndarray, reference: np.ndarray) -> np.ndarray:
        ...
        self.intensity_compensated_linear = self.intensity(
            original, mode="linear"
        ) - self.intensity(reference, mode="linear")
        self.intensity_compensated_db = self.intensity(
            original, mode="db"
        ) - self.intensity(reference, mode="db")
        self.field_compensated = self.calculate_efield(
    ...
    def phase_unwrapping(
        self, phase_wrapped, method: Literal["Fast 2D", "Kamui", "Numpy"] = "Fast 2D"
    ):
        """
        ...
        #     phase_unwrapped = phase_unwrapping_kamui_normal(phase_wrapped)
            ...
            raise ValueError(
                f"Unsupported phase unwrapping method: {method}. "
    ...
    def phase_to_height(self, phase, n_resin=None):
        ...
        delta_n = (
            n_resin - n_air
        )  # change in refractive index - only approximate values
